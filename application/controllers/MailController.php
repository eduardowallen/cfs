...
class MailController extends Controller {
    ...
    if ($mail == 'new') {
      // Creating a new mail template
			...
			$this->set('headline', 'New mail template (text/html)');
...
//        {
          $stmt = $this->db->prepare("DELETE FROM mail_content WHERE mail = ? AND language = ?");
...
//        }
    ...
    if ($mail == 'new') {
      // Creating a new mail template
			...
			$this->set('headline', 'New mail template (text/plain)');
			$this->set('mail_label_label', 'Mail template name (used in code to refer to this template)');
			$this->set('subject_label', 'Subject');
			$this->set('content_label', 'Content');
			$this->set('save_label', 'Save');
			...
			if (isset($_POST['save'])) {
				...
				//$stmt = $this->db->prepare("UPDATE mail_content SET subject = ?, content = ? WHERE mail = ? AND language = ?");
				$stmt = $this->db->prepare("INSERT INTO plain_mail_content SET subject = ?, content = ?, mail = ?, language = ?");
				if($stmt->execute(array($_POST['mail_subject'], $_POST['mail_content'], $_POST['mail_label'], $lang)))
        {
          // On success, redirect to editing the new mail template
          header('Location: '.BASE_URL.'mail/editPlain/'.$_POST['mail_label'].'/'.$lang);
          exit;
        }
...
/*
      // Attempt to update the mail, if it fails, assume the user is adding a translated version into the database
				$stmt = $this->db->prepare("UPDATE plain_mail_content SET subject = ?, content = ? WHERE mail = ? AND language = ?");
        $result = $stmt->execute(array($_POST['mail_subject'], $_POST['mail_content'], $mail, $lang));
				if($result === false || $stmt->rowCount() === 0)
        {*/
          $stmt = $this->db->prepare("DELETE FROM plain_mail_content WHERE mail = ? AND language = ?");
          $stmt->execute(array($mail, $lang));
          $stmt = $this->db->prepare("INSERT INTO plain_mail_content SET subject = ?, content = ?, mail = ?, language = ?");
          $stmt->execute(array($_POST['mail_subject'], $_POST['mail_content'], $mail, $lang));
        //}
			...
			$stmt = $this->Mail->db->prepare("SELECT * FROM plain_mail_content WHERE mail = ? AND language = ?");
			$stmt->execute(array($mail, $lang));
			$mailContent = $stmt->fetch(PDO::FETCH_ASSOC);
			$this->setNoTranslate('mail_subject', $mailContent['subject']);
			$this->setNoTranslate('mail_content', $mailContent['content']);
      ...
      // Fetch the list of mails
			$stmt = $this->Mail->db->prepare("SELECT mail, subject, content, language FROM plain_mail_content LEFT JOIN language on plain_mail_content.language = language.id ORDER BY mail ASC, `default` ASC");
			$stmt->execute(array());
			$results = $stmt->fetchAll(PDO::FETCH_ASSOC);
