<?php if (!isset($event_locked)) { ?>
	...
	<form action="fair/extraOptions/<?php echo $fair_id; ?>/edit/<?php echo $item; ?>" method="post" style="background:#efefef; border:1px solid #b1b1b1; padding:20px;">
		...
		<input required="yes" type="number" name="price" id="price" step="0.01" title="<?php echo uh($translator->{"Comma as delimiter is not accepted. Please use dot instead (eg: 234.53 = 234,53)."}); ?>" value="<?php echo $current_price ?>"/>
	...
	<form action="fair/extraOptions/<?php echo $fair_id; ?>" method="post" style="background:#efefef; border:1px solid #b1b1b1; padding:20px;">
		...
		<div class="column33">
		<label for="price"><?php echo $price_label; ?> *</label>
		<input required="yes" type="number" name="price" id="price" step="0.01" title="<?php echo uh($translator->{"Comma as delimiter is not accepted. Please use dot instead (eg: 234.53 = 234,53)."}); ?>" />
		</div>
		<div class="column33">
		<label for="vat"><?php echo $vat_label; ?></label>
