...
public function exportedFile() {
	...
	foreach ($files as $file) {
		...
		//$file = iconv("UTF-8","Windows-1252//IGNORE",$file);
		//encode to windows-1252 to save to the filesystem
		//$encoded_filename = iconv("UTF-8","Windows-1252//IGNORE",$file);
		if (file_exists($file)) {
			setlocale(LC_ALL,'sv_SE.UTF-8 UTF-8');
	  		$zip->addFile($file, iconv("UTF-8", "CP852", pathinfo( $file, PATHINFO_BASENAME )));
		}
	}
	$zip->close();
}
...
public function exportFiles() {
	...
	$zip->close();
	echo $filename;
	//header("Location: ".BASE_URL."downloadFile.php");
/*
	header('Content-Type: application/zip; charset=utf-8');
	header('Content-disposition: attachment; filename='.$filename);
